...
static void _crocodilo_update_position(Elements *self, int dx, int dy);
...
{
    crocodilo *pDerivedObj = malloc(sizeof(crocodilo));
    ...
    for (int i = 0; i < 3; ++i) {
        char buf[64];
        sprintf(buf, "assets/image/crocodilo_%s.png", state_str[i]);
    ...
    pObj->Draw        = crocodilo_draw;
    pObj->Update      = crocodilo_update;
    pObj->Interact    = crocodilo_interact;
    pObj->Destroy     = crocodilo_destory;
...
void crocodilo_update(Elements *self)
{
    ...
    if (dist > SHOT_RANGE) {
        ...
        _crocodilo_update_position(self, (int)vx, (int)vy);
        ch->dir   = (dx >= 0);
        ch->state = MOVE;
    } else {
        ch->state = STOP;
...
void crocodilo_interact(Elements *self) { /* 目前無需額外碰撞 */ }
